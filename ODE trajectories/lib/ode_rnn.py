...
# Modified by Futoon M. Abu Shaqra: Extracting the latent ODE trajectories
...
class ODE_RNN(Baseline):
	...
	def get_reconstruction(self, time_steps_to_predict, data, truth_time_steps, 
		...
		Avrglatent = torch.mean(latent_ys, dim=1)
		Avrglatent = Avrglatent[-1, :, :]  # Avrglatent[-1, :, :]

		#Avrglatent = Avrglatent.reshape(Avrglatent.shape[0], 100 * 10)
		latentClu = latent_ys[-1, :, :]
		AvrglatentN = Avrglatent.detach().numpy()
		if (run_models.x == 200):
			if (not run_models.IsTest):

				DataandLat = torch.cat([data, latentClu], -1)



				DataandLat = DataandLat.reshape(DataandLat.shape[0], DataandLat.shape[1] * DataandLat.shape[2])

				DataandLatandLab = torch.cat([DataandLat, label], -1)

				DataandLatandLab = DataandLatandLab.detach().numpy()

				DataandLat = DataandLat.detach().numpy()
				label1 = label.detach().numpy()

				with open("TRForAtt.csv", 'a', newline='') as csvfile:
					np.savetxt(csvfile, DataandLat, delimiter=",")
				with open("TRlabel.csv", 'a', newline='') as csvfile:
					np.savetxt(csvfile, label1, delimiter=",")

			if (run_models.IsTest):
				...
				DataandLat = DataandLat.detach().numpy()
				label1 = label.detach().numpy()
				with open("TSForAtt.csv", 'a', newline='') as csvfile:
					np.savetxt(csvfile, DataandLat, delimiter=",")
				with open("Tslabel.csv", 'a', newline='') as csvfile:
					np.savetxt(csvfile, label1, delimiter=",")
