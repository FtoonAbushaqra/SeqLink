...
def get_data_min_max(records):
	...
	for b, (record_id, tt, vals, mask, labels, GODEL4, GODEL3 ,GODEL2, GODEL1) in enumerate(records):
...
def variable_time_collate_fn(batch, args, device = torch.device("cpu"), data_type = "train", 
	...
	combined_GODEL4 = torch.zeros([len(batch), len(combined_tt), 10]).to(device)
	combined_GODEL3 = torch.zeros([len(batch), len(combined_tt), 10]).to(device)
	combined_GODEL2 = torch.zeros([len(batch), len(combined_tt), 10]).to(device)
	combined_GODEL1 = torch.zeros([len(batch), len(combined_tt), 10]).to(device)
	#print (combined_GODEL.shape)
	...
	for b, (record_id, tt, vals, mask, labels, GODEL4, GODEL3, GODEL2, GODEL1) in enumerate(batch):
		tt = tt.to(device)
		vals = vals.to(device)
		mask = mask.to(device)

		GODEL4 = GODEL4.to(device)
		GODEL3 = GODEL3.to(device)
		GODEL2 = GODEL2.to(device)
		GODEL1 = GODEL1.to(device)
		...
		combined_GODEL4[b, indices,] = GODEL4
		combined_GODEL3[b, indices,] = GODEL3
		combined_GODEL2[b, indices,] = GODEL2
		combined_GODEL1[b, indices,] = GODEL1
	...
	data_dict = {
		...
		"GODEL4": combined_GODEL4,
		"GODEL3": combined_GODEL3,
		"GODEL2": combined_GODEL2,
		"GODEL1": combined_GODEL1}
