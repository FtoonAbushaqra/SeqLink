...
###########################
# Modified by Futoon M. Abu Shaqra
import numpy as np
import torch
...
class GRU_unit(nn.Module):
	...
	def forward(self, y_mean, y_mean4, y_mean3, y_mean2, y_mean1, y_std, x, masked_update=True):
		y_concat = torch.cat([y_mean, y_std, x], -1)
		y_mean4 = y_mean4 * 0.05
		y_mean3 = y_mean3 * 0.15
		y_mean2 = y_mean2 * 0.3
		y_mean1 = y_mean1 * 0.5
		y_meanFL = y_mean4.add(y_mean3)
		y_meanFL = y_meanFL.add(y_mean2)
		y_meanFL = y_meanFL.add(y_mean1)
		# y_meanFL = y_meanFL / 4
		y_meanF = y_meanFL.add(y_mean)
		y_meanF = y_meanF / 2
		y_concat = torch.cat([y_meanF, y_std, x], -1)

		update_gate = self.update_gate(y_concat)
		reset_gate = self.reset_gate(y_concat)
		concat = torch.cat([y_meanF * reset_gate, y_std * reset_gate, x], -1)
		
		new_state, new_state_std = utils.split_last_dim(self.new_state_net(concat))
		new_state_std = new_state_std.abs()

		new_y = (1-update_gate) * new_state + update_gate * y_meanF
...
class Encoder_z0_ODE_RNN(nn.Module):
	...
	def forward(self, data,data_and_mask_and_latent, time_steps, run_backwards = True, save_info = False):
		...
		if len(time_steps) == 1:
			...
			last_yi, last_yi_std, _, extra_info = self.run_odernn(
				data, data_and_mask_and_latent, time_steps, run_backwards = run_backwards,
				save_info = save_info)
	...
	# Modified by Futoon M. Abu Shaqra: Added the latent ODE trajectories
	def run_odernn(self, data, data_and_mask_and_latent, time_steps,
		...
		for i in time_points_iter:
			...
			xiN = data_and_mask_and_latent[:, i, :].unsqueeze(0)
			AvargeYode4 = xiN[:, :, 0:10]
			AvargeYode3 = xiN[:, :, 10:20]
			AvargeYode2 = xiN[:, :, 20:30]
			AvargeYode1 = xiN[:, :, 30:40]

			yi, yi_std = self.GRU_update(yi_ode, AvargeYode4,AvargeYode3,AvargeYode2,AvargeYode1 ,  prev_std, xi)

			prev_y, prev_std = yi, yi_std			
			prev_t, t_i = time_steps[i],  time_steps[i-1]
			ys_ode.append(yi_ode)
			latent_ys.append(yi)
